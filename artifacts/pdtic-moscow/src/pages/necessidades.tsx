            ...
            <tbody className="divide-y divide-border">
              {filtered.map((n) => (
                <tr key={n.id} className="hover:bg-muted/20 transition-colors" data-testid={`row-necessidade-${n.id}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${MOSCOW_COLORS[n.classificacao_moscow] ?? ""}`}>
                      {MOSCOW_LABELS[n.classificacao_moscow] ?? n.classificacao_moscow}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[n.status] ?? ""}`}>
